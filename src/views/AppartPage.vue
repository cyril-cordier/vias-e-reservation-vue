<template>
  <div class="appartPage">
    ...
    <div v-if="getUserMe.profile.who_is == 'ND'">
      <div class="mt-20 text-center font-bold text-xl text-blue-800 my-3">
        Votre compte n'a pas encoré été validé par le gestionnaire.
      </div>
      <div class="my-10 text-center font-bold text-l text-blue-800 my-3">
        Vous serez informé par mail sur l'adresse
        {{ getUserMe.profile.email }} dès que votre compte sera actif.
      </div>
        ...
        <div class="container px-5 py-10 mx-auto">
          ...
          <div class="lg:w-4/5 mx-auto flex flex-wrap">
            <div class="w-1/6">
              <!-- MODAL IMAGE -->
              <div
                v-for="image in getAppartById.appart.images.slice(
                  1,
                  getAppartById.appart.images.length
                )"
                :key="image.id"
              >
                <img
                  :src="image"
                  alt=""
                  class="w-1/2 border mx-auto m-1"
                  @click="Modalimg = !Modalimg"
                />
              </div>
              <div
                class="fixed overflow-x-hidden overflow-y-auto inset-0 flex justify-center items-center z-50"
              ...
              >
                <div class="relative mx-auto w-3/4 max-w-full max-h-full">
                  <div
                    class="bg-opacity-25 bg-blue-900 w-full rounded shadow-2xl flex flex-col"
                  >
            ...
            <div class="lg:w-1/2 w-full lg:pl-10 lg:py-4 mt-2 lg:mt-0">
              <div class="p-1 W-1/3">
                <div class="flex items-center">
                  <button
                    @click="(isClickAvis = !isClickAvis), (isClick = false)"
                  >
                    <div class="text-blue-600 ml-3">
                      {{ reviewOfAppart.length }} Avis
                    </div>
                  </button>
                </div>
              ...
              <p
                ...
                v-if="getAppartById.appart.available == true"
              >
              ...
              <div class="flex mb-2">
                ...
                <div class="font-bold text-orange-500 mx-2">
                  Carol PRAVDA-STAROV
                </div>
              </div>
              <div
              ...
              ></div>
              <div
                class="flex mt-2 items-center pb-5 border-b-2 border-blue-200 mb-3"
              ></div>
              <div class="flex content-around items-center">
                ...
                <button
                  v-if="
                    getUserMe.profile != null &&
                    getAppartById.appart.available == true
                  "
            ...
            <div
              v-if="getCreateReservationResponse.success"
            ...
            >
              <div class="flex">
                ...
                <div>
                  <p class="font-bold">
                    Votre demande de reservation vient d'etre transmise au
                    gestionnaire. Celui ci prendra une decision dans les plus
                    brefs delais !
                  </p>
                </div>
              </div>
            ...
            <div v-if="this.error == true" role="alert" class="w-full m-5">
              ...
              <div
                class="text-center border border-t-0 border-red-400 rounded-b bg-red-100 px-4 py-3 text-red-700"
              >
                <p>
                  Une demande est en attente pour cette date merci de bien
                  vouloir choisir une autre date.
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div
                class="md:w-2/3 lg:w-1/2 w-full mx-auto block border-r border-l border-b rounded-b border-blue-200 shadow"
              >
                <div
                  class="block bg-white text-blue-700 text-sm font-bold text-center"
                >
                  <table class="min-w-full">
                    ...
                    <tbody class="bg-white">
                      <tr>
                        <td
                          class="py-2 whitespace-no-wrap border-b border-blue-200"
                        >
                          <div
                            class="flex items-center justify-around text-xs leading-5 font-medium text-blue-900"
                          >
                            30 €/j
                          </div>
                        </td>
                      ...
                      <tr>
                        <td
                          class="px-2 py-2 whitespace-no-wrap border-b border-blue-200"
                        >
                          <div
                            class="flex items-center justify-around text-xs leading-5 font-medium text-blue-900"
                          >
            ...
            >
              <form
                @submit.prevent="reservationForm()"
                class="md:w-2/3 lg:w-1/2 w-full border p-3 bg-blue-200 mx-auto"
              >
                <label
                  class="block text-blue-700 text-lg font-bold mb-2 text-center"
                  for="nomdujeu"
                >
                  J'aimerai louer l'appartement :
                  {{ getAppartById.appart.name }} </label
                ><label
            ...
            >
              <div class="flex flex-col w-full bg-blue-200">
                <form
                  ...
                  v-if="getUserMe.profile != null"
                  class="border p-3 m-4 bg-white"
                  action=""
                >
                  <!-- MESSAGE CREATE SUCCESS  -->
                  <div
                    v-if="createReviewSuccess != null"
                    class="bg-teal-100 border-t-4 mt-5 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md"
                    role="alert"
                  >
                    <div class="flex">
                      ...
                      <div>
                        <p class="font-bold">
                          Votre avis vient d'etre publié avec succès
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <textarea
                    ...
                    placeholder="Saisir ici votre commentaire"
                  ></textarea>
                ...
                <div
                  v-for="review in reviewOfAppart"
                  :key="review._id"
                  class="border p-3"
                >
                  <div class="flex">
                    <div class="w-11/12">
                      <div class="text-m title-font text-gray-600">
                        Commentaire de : {{ review.userId.username }}
                      </div>
                      <div class="text-xs title-font text-gray-600">
                        Publié le : {{ review.created_at }}
                      </div>
                    </div>
                    <div class="w-1/12 align-content: flex-end;">
                      <svg
                        ...
                        stroke="currentColor"
                        class="w-5"
                        v-if="getUserMe.profile._id == review.userId._id"
                  ...
                  <div class="mt-1 p-1 rounded bg-white">
                    {{ review.comment }}
                  </div>
                </div>
              </div>
            ...
            <div class="Calendar w-full h-full">
              <h1 class="text-center m-5 font-bold">
...
export default {
  ...
  methods: {
    ...
    tarif() {
      ...
      var from =
        new Date(debut[0], parseInt(debut[1]) - 1, debut[2]).getTime() /
        86400000;
      var to =
        new Date(fin[0], parseInt(fin[1]) - 1, fin[2]).getTime() / 86400000;
      this.diff = parseInt(new Number(to - from).toFixed(0));
      this.montant = 0;
      ...
      for (var i = 1; i <= this.diff; i++) {
        ...
        if (ladate.substr(5, 2) >= 9 || ladate.substr(5, 2) <= 6) {
          ...
          (ladate.substr(5, 2) == 7 && ladate.substr(8, 2) < 15) ||
          (ladate.substr(5, 2) == 8 && ladate.substr(8, 2) > 15)
        ) {
    ...
    reservationForm() {
      ...
      if (
        ...
        var demandeResaParams = {
          ...
          message:
            this.getUserMe.profile.username +
            " aimerait louer l'appartement " +
            this.diff +
            " jours, du " +
            locStart[2] +
            "/" +
        ...
        emailjs
          .send(
            ...
            this.email_template.demandeReservation,
            demandeResaParams,
