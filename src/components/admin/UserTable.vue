              ...
              <tr v-for="user in getAllUsers.users" :key="user._id">
                ...
                >
                  <button
                    ...
                    v-if="
                      getUserMe.profile.who_is == 'Administrateur' ||
                      (getUserMe.profile.who_is == 'Gestionnaire' &&
                        user.is_admin == false) ||
                      getUserMe.profile._id == user._id
                    "
                  >
                    Modification
                  </button>
    ...
    editUserSubmit() {
      ...
      if (this.userToEdit.who_is != "ND") {
        var activeParams = {
          ...
          message:
            "Votre compte a été validé, vous pouvez accéder à l'espace de réservation : " +
            process.env.VUE_APP_URL +
            "/appartements/5fa066cbdb3ee70017fa88ed",
          to_email: this.userToEdit.email,
        };

        emailjs
          .send(
            process.env.VUE_APP_SERVICE_ID_RESPONSE,
            this.email_template.informUser,
            activeParams,
            process.env.VUE_APP_USER_ID_RESPONSE
          )
          .then(
            (result) => {
